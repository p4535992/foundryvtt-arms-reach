...
class GeometricUtils {
  ...
  static Rotated(pPosition, protation) {
    return [
      Math.cos(cGradtoRad * protation) * pPosition[0] - Math.sin(cGradtoRad * protation) * pPosition[1],
      Math.sin(cGradtoRad * protation) * pPosition[0] + Math.cos(cGradtoRad * protation) * pPosition[1],
    ];
  }
  ...
  static scaletoxy(pVector, pfactorarray) {
    return GeometricUtils.scalexy(
      GeometricUtils.norm(
        GeometricUtils.scalexy(
          pVector,
          pfactorarray.map((vvalue) => 1 / vvalue)
        )
      ),
      pfactorarray
    );
  ...
  static scaledDistance(pPositionA, pPositionB, pfactorarray, protation = 0) {
    if (!protation) {
      return GeometricUtils.value(
        GeometricUtils.scalexy(GeometricUtils.Difference(pPositionA, pPositionB), pfactorarray)
      );
    } else {
      return GeometricUtils.value(
        GeometricUtils.scalexy(
          GeometricUtils.Rotated(GeometricUtils.Difference(pPositionA, pPositionB), protation),
          pfactorarray
        )
      );
    }
  ...
  static TokenDistance(pTokenA, pTokenB) {
    if (pTokenA && pTokenB) {
      return (
        (Math.sqrt(
          (pTokenA.x +
            GeometricUtils.insceneWidth(pTokenA) / 2 -
            (pTokenB.x + GeometricUtils.insceneWidth(pTokenB) / 2)) **
            2 +
            (pTokenA.y +
              GeometricUtils.insceneHeight(pTokenA) / 2 -
              (pTokenB.y + GeometricUtils.insceneHeight(pTokenB) / 2)) **
              2
        ) /
  ...
  static TokenBorderDistance(pTokenA, pTokenB) {
    if (pTokenA && pTokenB) {
      let vDistance =
        GeometricUtils.TokenDistance(pTokenA, pTokenB) -
        (Math.max(
          GeometricUtils.insceneWidth(pTokenA) + GeometricUtils.insceneWidth(pTokenB),
          GeometricUtils.insceneHeight(pTokenA) + GeometricUtils.insceneHeight(pTokenB)
        ) /
          2 /
          canvas.scene.dimensions.size) *
          canvas.scene.dimensions.distance;
  ...
  static sortbymaxdim(pTokens) {
    let vsortedTokens = pTokens.sort(function (vTokena, vTokenb) {
      return Math.max(vTokena.height, vTokena.width) - Math.max(vTokenb.height, vTokenb.width);
    });
    ...
    switch (pTokenForm) {
      case cTokenFormCircle:
        if (Math.max(GeometricUtils.insceneWidth(pToken) == GeometricUtils.insceneHeight(pToken))) {
          ...
          return GeometricUtils.scaletoxy(vDirection, [
            GeometricUtils.insceneWidth(pToken) / 2,
            GeometricUtils.insceneHeight(pToken) / 2,
          ]);
        }
      ...
      case cTokenFormRectangle:
        ...
        let vxBorder =
          Math.abs(vDirection[0]) / GeometricUtils.insceneWidth(pToken) >
          Math.abs(vDirection[1]) / GeometricUtils.insceneHeight(pToken);
  ...
  static withinBoundaries(pToken, pTokenForm, pPosition) {
    switch (pTokenForm) {
      case cTokenFormCircle:
        if (Math.max(GeometricUtils.insceneWidth(pToken) == GeometricUtils.insceneHeight(pToken))) {
          return (
            GeometricUtils.Distance(GeometricUtils.CenterPosition(pToken), pPosition) <=
            Math.max(GeometricUtils.insceneWidth(pToken)) / 2
          );
        } else {
          //supports ellipses through scaling
          return (
            GeometricUtils.scaledDistance(
              GeometricUtils.CenterPosition(pToken),
              pPosition,
              [1 / GeometricUtils.insceneWidth(pToken), 1 / GeometricUtils.insceneHeight(pToken)],
              -pToken.rotation
            ) <=
            1 / 2
          );
      ...
      case cTokenFormRectangle:
        ...
        return (
          Math.abs(vDifference[0]) <= GeometricUtils.insceneWidth(pToken) / 2 &&
          Math.abs(vDifference[1]) <= GeometricUtils.insceneHeight(pToken) / 2
        );
    ...
    switch (pGrid.type) {
      ...
      case 1:
        ...
        for (let dim = cxid; dim <= cyid; dim++) {
          ...
          vsnapposition[dim] =
            Math.sign(ppositon[dim]) *
            (Math.round((Math.abs(ppositon[dim]) - voffset - 1) / pGrid.size) * pGrid.size + voffset);
        }
