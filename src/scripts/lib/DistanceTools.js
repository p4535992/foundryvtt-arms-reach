export default class DistanceTools {
    ...
    static xyData(ObjectDoc) {
        ...
        for (let key of ["x", "y"]) {
            ...
            if (adjustxyz && data[DistanceTools.DSK[key]]) {
		...
		if (data.rotation) {
			...
			data.width = Math.sqrt((Math.cos(DistanceTools.DEGtoRAD * data.rotation) * width)**2 + (Math.sin(DistanceTools.DEGtoRAD * data.rotation) * height)**2);
			data.height = Math.sqrt((Math.cos(DistanceTools.DEGtoRAD * data.rotation) * height)**2 + (Math.sin(DistanceTools.DEGtoRAD * data.rotation) * width)**2);
		}
		*/
