...
function distance_between(a, b) {
    ...
    function _euclideanCalculation(p1, p2) {
        ...
        const d =
            Math.sqrt(Math.pow(p2.x - p1.x, 2) + Math.pow(p2.y - p1.y, 2) + Math.pow(p2.z - p1.z, 2)) / unitsToPixel;
        return d;
    }
...
function _units_between_placeable_and_placeable(aArmsReachData, bArmsReachData) {
    ...
    const dist =
        (_getUnitTokenDistUniversal(aArmsReachData, bArmsReachData) * canvas.dimensions.size) /
...
function _getUnitTokenDistUniversal(aArmsReachData, bArmsReachData) {
    ...
    if (game.Levels3DPreview?._active) {
        ...
        const d =
            Math.sqrt(Math.pow(x2 - x1, 2) + Math.pow(y2 - y1, 2) + Math.pow((z2 - z1) * unitsToPixel, 2)) /
            unitsToPixel;
        return d;
    }
}
