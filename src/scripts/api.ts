...
const API = {
	...
	isReachableByTag(token: Token, tag: string, maxDistance?: number, useGrid?: boolean, userId?: string): boolean {
		...
		if (!(<boolean>game.modules.get(CONSTANTS.TAGGER_MODULE_NAME)?.active)) {
			warn(
				`The module '${CONSTANTS.TAGGER_MODULE_NAME}' is not active can't use the API 'isReachableByTag'`,
				true
			);
			return false;
		} else {
	...
	isReachable(
		...
		if (relevantDocument instanceof TokenDocument) {
			...
			const doorControlTarget: DoorControl = <DoorControl>canvas.controls?.doors?.children.find(
			...
			isInReach = DrawingsReach.globalInteractionDistance(
				token,
				drawingTarget,
				maxDistance,
				useGrid,
