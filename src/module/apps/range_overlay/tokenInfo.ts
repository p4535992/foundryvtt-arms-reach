...
export class TokenInfo {
  ...
  get weaponRange() {
    return <number>this.token.document.getFlag(CONSTANTS.MODULE_NAME,CONSTANTS.FLAG_NAMES.WEAPON_RANGE) || CONSTANTS.DEFAULT_WEAPON_RANGE;
  }
  ...
  getSpeedFromAttributes() {
    ...
    if (game.system.id === "pf1" || game.system.id === "D35E") {
      ...
      otherSpeeds = <number[]>Object.entries(actorAttrs.movement).filter(s => typeof(s[1]) === "number").map(s => s[1]);
    }
